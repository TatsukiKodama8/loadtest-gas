...
## 2026-02-10: 構成定義のリファクタリング (型安全性の向上)

### 目的
- `src/definitions.ts` の `Targets` および `Metrics` を配列からオブジェクト（Record）形式に変更し、冗長なキー定義を削減する。
- TypeScript の `as any` キャストを排除し、コンパイル時の型チェックを強化する。
- ターゲットと出力列のマッピングにおける整合性を担保する。

### リファクタリング内容
- [x] `src/definitions.ts`: `TargetLabels` インターフェースの定義。
- [x] `src/definitions.ts`: `Targets` を `Record<TargetKey, TargetLabels>` に変更。
- [x] `src/definitions.ts`: `Metrics` を `Record<MetricKey, MetricDefinition>` に変更。
